...
#Πρέπει να γίνεται γρήγορα με ρυθμό από 2 έως 10 δείγματα / sec
def chk_state():
    global tiks
    fn.button1.tick()  #Έλεγξε αν πατήθηκε το κουμπί 1
    fn.bell_btn.tick()
    #Αν έχει περάσει 1 sec να ελέγξει τους αισθητήρες
    if tiks >= df.TIMES_TO_SEC - 1:
        tiks = 0
        read_sensor()
    else:
        tiks += 1

#Αρχικοποίση των timers και ανάθεση περιόδου και function που θα καλούν
timer1.init(period=1000, mode=machine.Timer.PERIODIC, callback=lambda t:toggle(df.WiFi_LED))
timer2.init(period=df.TIK_TIME, mode=machine.Timer.PERIODIC, callback=lambda t:chk_state()) #125 ms

#Συνάρτηση Call Back όταν γίνει publish σε κάποιο από τα topics που η κάρτα έχει συνδρομή
def sub_cb(topic, msg):
    print((topic, msg))
    #Ψηφιακά topics
    for i_topic, pin in zip(df.d_topics, df.d_pins):
        if i_topic in topic:
            if msg == b"ON":
                fn.digitalWrite(pin, df.HIGH)
            elif msg == b"OFF":
                fn.digitalWrite(pin, df.LOW)
    #Αναλογικά PWM topics
    for i_topic, pin in zip(df.a_topics, df.a_pins):
        if i_topic in topic:
            fn.analogWrite(pin, int(msg))
    if df.S_TOPIC1 in topic:
        fn.servoWrite(8, int(msg))
    if df.S_TOPIC2 in topic:
        fn.servoWrite(14, int(msg))

#Για πάντα έλεγξε αν υπάρχουν συνδέσεις και στείλε περιεχόμενο σε browser
while True:
    df.c = MQTTClient(CLIENT_ID, SERVER) #Στιγμιότυπο του Client
    df.c.set_callback(sub_cb) #Συνάρτηση callback
    df.c.connect() #Σύνδεση στον MQTT server
    print("Connected to server: %s" % (SERVER))
    for topic in df.d_topics:
        df.c.subscribe(topic)
        print("Subscribed to %s topic" % (topic))
    for topic in df.a_topics:
        df.c.subscribe(topic)
        print("Subscribed to %s topic" % (topic))
    df.c.subscribe(df.S_TOPIC1)
    df.c.subscribe(df.S_TOPIC2)
    #Για πάντα περίμενε μηνύματα
    try:
        while 1:
